// Zak Strassberg MEAN project week - Tetris
// Built upon Jake Gordon's tetris implementation (MIT license)
...
speed   = { start: 0.6, decrement: 0.005, min: 0.2 }, // how long before piece drops by 1 row (seconds)
...
// Tetrominos:
// each element represents a rotation of the piece (0, 90, 180, 270)
...
var xx = { size: 2, blocks: [0xCC00, 0xCC00, 0xCC00, 0xCC00], color: 'grey' };
...
function run() {
  ...
  function frame() {
    ...
    // using requestAnimationFrame have to be able to
    // handle large delta's caused when it 'hibernates'
    // in a background or non-visible tab
    update(Math.min(1, (now - last) / 1000.0));         draw();
    stats.update();
    last = now;
...
function addLine() {
  ...
  if (!gap) {
    setBlock(Math.floor(Math.random()*10), ny-1, undefined)
  }
}

function moveAllBlocksUp() {
  for (var y = 0; y < ny; ++y) {
    for (var x = 0; x < nx; ++x) {
